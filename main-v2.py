...
#Verifica se existe as tabelas, se não cria uma#
ver = cursor.execute("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='dados_1';").fetchone()
if ver[0] == 0:
    cursor.execute("CREATE TABLE dados_1 (dados text)")

ver = cursor.execute("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='dados_2';").fetchone()
if ver[0] == 0:
    cursor.execute("CREATE TABLE dados_2 (dados text)")

ver = cursor.execute("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='dados_3';").fetchone()
if ver[0] == 0:
    cursor.execute("CREATE TABLE dados_3 (dados text)")
    ...
    while n1 == "4" and len(lista1+lista2+lista3) >= 1:
        ...
        while apag not in ("1", "2", "3"):
            ...
            if apag not in ('1', '2', '3'):
                print(f'"{apag}" Não corresponde a uma lista!')
